...
describe('resolveImage', () => {
    it('resolves mainImage __ref to RECORD_MAIN url', () => {
    ...
    it('returns null when mainImage is undefined', () => {
        expect(resolveImage({}, {})).toBeNull();
    });

    it('returns null when Image ref is not in apolloState', () => {
        const advert = { mainImage: { __ref: 'Image:404' } };
        expect(resolveImage(advert, {})).toBeNull();
    });

    it('returns null when Image object has no RECORD_MAIN key', () => {
