...
describe('enrich', () => {
    it('marks new listing as isNew=true with daysOnMarket=0', () => {
        const { enriched } = enrich([base], {});
        expect(enriched[0].isNew).toBe(true);
        expect(enriched[0].daysOnMarket).toBe(0);
        expect(enriched[0].priceChanged).toBe(false);
        expect(enriched[0].previousPrice).toBeNull();
    });

    it('marks known listing as isNew=false', () => {
        const history = { sreality_1: { price: 5_000_000, firstSeenAt: new Date().toISOString() } };
        const { enriched } = enrich([base], history);
        expect(enriched[0].isNew).toBe(false);
    });

    it('detects price change and stores previousPrice', () => {
        const history = { sreality_1: { price: 6_000_000, firstSeenAt: new Date().toISOString() } };
        const { enriched } = enrich([base], history);
        expect(enriched[0].priceChanged).toBe(true);
        expect(enriched[0].previousPrice).toBe(6_000_000);
    });

    it('does not flag priceChanged when price is unchanged', () => {
        const history = { sreality_1: { price: 5_000_000, firstSeenAt: new Date().toISOString() } };
        const { enriched } = enrich([base], history);
        expect(enriched[0].priceChanged).toBe(false);
    });

    it('flags isBestDeal for listing 15% below median pricePerSqm', () => {
    ...
    it('does not flag isBestDeal when pricePerSqm is null', () => {
        const { enriched } = enrich([{ ...base, pricePerSqm: null }], {});
        expect(enriched[0].isBestDeal).toBe(false);
    });

    it('preserves firstSeenAt from existing history', () => {
        const firstSeenAt = '2024-01-01T00:00:00.000Z';
        const history = { sreality_1: { price: 5_000_000, firstSeenAt } };
        const { updatedHistory } = enrich([base], history);
        expect(updatedHistory.sreality_1.firstSeenAt).toBe(firstSeenAt);
    });

    it('calculates daysOnMarket > 0 for listing seen in the past', () => {
        const firstSeenAt = new Date(Date.now() - 3 * 86_400_000).toISOString(); // 3 days ago
        const history = { sreality_1: { price: 5_000_000, firstSeenAt } };
        const { enriched } = enrich([base], history);
        expect(enriched[0].daysOnMarket).toBe(3);
    });

    it('sets firstSeenAt in updatedHistory for a new listing', () => {
        const before = Date.now();
        const { updatedHistory } = enrich([base], {});
        const after = Date.now();
        const firstSeenAt = new Date(updatedHistory.sreality_1.firstSeenAt).getTime();
        expect(firstSeenAt).toBeGreaterThanOrEqual(before);
        expect(firstSeenAt).toBeLessThanOrEqual(after);
    });

    it('returns empty enriched array for empty input', () => {
