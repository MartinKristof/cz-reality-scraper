...
export const calcPerPortal = (maxItems: number | null, portalCount: number): number => {
    if (portalCount <= 0) return 0;
    if (maxItems === null) return Infinity;

    return Math.ceil(maxItems / portalCount);
};
